    ...
    <div id="page-form" class="w-11/12 md:w-2/3 mt-10 p-10 bg-white border rounded-xl mb-10">
        <form action="/editData" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                ...
                <input type="email" name="email" class="w-full border rounded px-5 py-3 mt-3" placeholder="emailku@domain.com" value="<?= $userData['email'] ?>">
            </div>
            <div class="mb-5">
                <label for="password" class="font-semibold">Password</label>
                <input type="password" name="password" class="w-full border rounded px-5 py-3 mt-3" placeholder="Tulis Password Anda" value="<?= $userData['password'] ?>">
            </div>
            <div class="mb-5">
                <label for="full_name" class="font-semibold">Nama Lengkap</label>
                <input type="text" name="full_name" class="w-full border rounded px-5 py-3 mt-3" placeholder="John Doe" value="<?= $userData['full_name'] ?>">
            </div>
            <div class="mb-5">
                <label for="address" class="font-semibold">Alamat</label>
                <input type="text" name="address" class="w-full border rounded px-5 py-3 mt-3" placeholder="Jl Ptimura No 101" value="<?= $userData['address'] ?>">
            </div>
            <div class="mb-5">
                <label for="gender" class="font-semibold">Jenis Kelamin</label>
                <select name="gender" class="w-full border rounded px-5 py-3 mt-3">
