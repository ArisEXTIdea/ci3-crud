<div id="content-wrapper" class="reltive flex flex-col items-center">
    ...
    <div id="content-header" class="w-full py-5 bg-gray-500">
        <h1 class="text-white text-center text-2xl font-bold">SISTEM INFORMASI USER</h1>
    </div>
    ...
    <?php endif;?>
    <div id="page-table" class="w-11/12 md:w-2/3 mt-10 p-10 bg-white border rounded-xl mb-10">
        <table class="w-full">
            <thead>
                <tr class="bg-violet-500">
                    <th class="py-5 text-white">No</th>
                    <th class="py-5 text-white">Nama</th>
                    <th class="py-5 text-white">Eamil</th>
                    <th class="py-5 text-white"><i class="fas fa-cogs"></i></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach($usersData as $index => $d):?>
                <tr>
                    <td class="border-b py-5 text-center"><?= $index + 1 ?></td>
                    <td class="border-b py-5"><?= $d['full_name'] ?></td>
                    <td class="border-b py-5"><?= $d['email'] ?></td>
                    <td class="border-b py-5 text-center">
                        <button class="py-3 px-3 mx-1 rounded text-white bg-red-400 hover:bg-red-500" onclick="handleModalShow('<?= $d['uid']?>')">
                            <i class="fas fa-trash"></i>
                        </button>
                        <a href="/detail-akun/<?= $d['uid']?>" class="py-3 px-3 mx-1 rounded text-white bg-sky-400 hover:bg-sky-500">
